...
## 15. Git и исследование Beget DBaaS (облачная БД отложена) — 2026-09-15
...
### Текущее решение: PostgreSQL + PostGIS под собственным управлением

Владелец выбрал обычную БД на старте; облачное размещение рассматривается в будущем при большом количестве пользователей. Планируем отдельный контейнер PostgreSQL + PostGIS, постоянный volume, отдельные роли приложения и миграций, закрытый сетевой доступ. Размещение на 159.194.241.77 зависит от проверки ресурсов, российского региона и изоляции соседних проектов; сервер пока не настроен.

Обновления PostgreSQL/PostGIS, мониторинг, резервное копирование вне сервера и проверка восстановления входят в нашу эксплуатацию. Сохраняются требования раздела 10 и задачи G1–G3; отказ от DBaaS не отменяет резервное копирование и восстановление.

Подключение задаётся через окружение, миграции используют стандартные PostgreSQL/PostGIS возможности. Возвращаемся к выбору облака по измеренным задержкам, росту хранилища, нагрузке и стоимости эксплуатации. Будущий перенос включает проверку совместимости расширений, тестовую миграцию, сверку данных, согласованное окно переключения/репликацию и план отката; простая смена адреса БД не переносит данные.

### Справочное исследование Beget — не текущая схема развёртывания

Ранее рассмотренная схема на будущее: Laravel API/workers/WebSocket/Redis на сервере приложения, PostgreSQL+PostGIS в Beget DBaaS в РФ. Приложение мобильного пользователя не подключается к БД напрямую. Локальная разработка использует PostGIS в Compose; production подключение задаётся окружением. Это меняет размещение, но не доменную схему.
...
При будущем рассмотрении облака отсутствие PostGIS или приемлемого восстановления исключает провайдера; остаёмся на собственной БД либо оцениваем другой managed provider. Daily dump сам по себе не выполняет предложенный RPO ≤1 часа. Beget не выбран для старта; услуги не созданы и не оплачены.
